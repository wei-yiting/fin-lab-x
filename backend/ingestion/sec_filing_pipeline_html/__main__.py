...
Usage:
    uv run python -m backend.ingestion.sec_filing_pipeline_html AAPL 10-K
    uv run python -m backend.ingestion.sec_filing_pipeline_html AAPL 10-K --fiscal-year 2025
    uv run python -m backend.ingestion.sec_filing_pipeline_html AAPL 10-K --force
    uv run python -m backend.ingestion.sec_filing_pipeline_html AAPL 10-K --verbose
    uv run python -m backend.ingestion.sec_filing_pipeline_html AAPL 10-K --json
    uv run python -m backend.ingestion.sec_filing_pipeline_html batch NVDA AAPL TSLA --filing-type 10-K
"""
...
usage: python -m backend.ingestion.sec_filing_pipeline_html TICKER FILING_TYPE [options]
       python -m backend.ingestion.sec_filing_pipeline_html batch TICKER... --filing-type TYPE [options]
...
Run with -h after a command for detailed options:
  python -m backend.ingestion.sec_filing_pipeline_html AAPL 10-K -h
  python -m backend.ingestion.sec_filing_pipeline_html batch -h
"""
...
def _run_single(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m backend.ingestion.sec_filing_pipeline_html",
...
def _run_batch(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m backend.ingestion.sec_filing_pipeline_html batch",
