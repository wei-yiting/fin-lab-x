...
from backend.ingestion.sec_filing_pipeline_html.html_preprocessor import (
    HTMLPreprocessor,
)
from backend.ingestion.sec_filing_pipeline_html.html_to_md_converter import (
    HtmlToMarkdownAdapter,
    MarkdownifyAdapter,
)
from backend.ingestion.sec_filing_pipeline_html.markdown_cleaner import MarkdownCleaner
from backend.ingestion.sec_filing_pipeline_html.pipeline import (
    SECFilingPipeline,
)
from backend.ingestion.sec_filing_pipeline_html.sec_downloader import SECDownloader
...
class TestBatchRetryTransientErrors:
    @patch("backend.ingestion.sec_filing_pipeline_html.pipeline.time.sleep")
...
class TestCreateClassMethod:
    @patch("backend.ingestion.sec_filing_pipeline_html.pipeline.SECDownloader")
    @patch("backend.ingestion.sec_filing_pipeline_html.pipeline.HTMLPreprocessor")
    @patch("backend.ingestion.sec_filing_pipeline_html.pipeline.create_converter")
    @patch("backend.ingestion.sec_filing_pipeline_html.pipeline.MarkdownifyAdapter")
    @patch("backend.ingestion.sec_filing_pipeline_html.pipeline.LocalFilingStore")
...
class TestIntegration:
    ...
    def test_j_prep_01_html_preprocessing_quality(self):
        ...
        from backend.ingestion.sec_filing_pipeline_html.sec_downloader import (
            SECDownloader,
        )
