...
- `adr/`: Architecture Decision Records — one file per decision (`NNNN-slug.md`: decision + rejected alternatives + why). Opens with a 1–2 sentence decision statement; targets one page (~500 words) — overflow means split or link out, not compress; a wicked problem may exceed the target with reviewer judgment. A reversed decision gets a new ADR superseding the old by number; envelope-reduced robustness cites `design-envelope.md` §9.
