...
| Concurrent JIT of same ticker | May happen | *First-time* ingests may run concurrently: duplicate work converges on deterministic IDs. Same-ticker **refresh** never runs concurrently — coalesced/rejected by an in-process in-flight check (§2). No distributed locks, no job queues. |
...
- **JIT failure semantics: legible failure is the standard.** Arbitrary tickers WILL hit unsupported cases (foreign filers, missing XBRL, no EDGAR presence). Required behavior: a structured, user-facing explanation of what failed and why. Silent partial/empty answers are bugs; exhaustively *handling* every filing variant is over-engineering. Ingestion is all-or-nothing per ticker — a failed ingest leaves no partial retrievable state. The sanctioned mechanism is **write-then-commit-marker**: upsert content first, write the completion marker last, and retrieval treats only marker-complete tickers as present. **Refresh of an already-committed ticker is wipe-before-rerun**: delete the marker first (the ticker legibly reads as not-ingested while work is in flight), wipe, re-ingest, re-mark — "all-or-nothing" means *committed or absent*, never a mix of old and new generations. Wipe-before-rerun is **not** concurrency-safe, so same-ticker refresh never runs concurrently: a refresh requested while one is in flight is coalesced or rejected by a simple in-process in-flight check — the same tier of mechanism as the session busy-guard, not a distributed lock or queue. With that, all-or-nothing holds mechanically: concurrent *first-time* ingests converge on deterministic IDs with no coordination, refreshes are serialized in-process, and no cross-store transactions or version/generation machinery are needed.
