...
graph LR
    ...
    subgraph "Downstream"
        D2[S3FilingStore] -.-> D
        E["sec_dense_pipeline_html<br/>chunk + embed + upsert"] --> F[Qdrant]
    end
...
| Store | `filing_store.py` | Persists `.md` files with YAML frontmatter at `data/sec_filings_html/{TICKER}/10-K/{fiscal_year}.md`. Atomic writes via temp file + `os.replace`. |
...
Operator-facing CLI for running the heuristic ladder against real EDGAR filings — see `python -m backend.ingestion.sec_filing_pipeline_html.validation --help` for hard-gate, discovery, and bootstrap-baseline modes.
...
uv run python -m backend.ingestion.sec_filing_pipeline_html AAPL 10-K --fiscal-year 2024 --force

# Batch download
uv run python -m backend.ingestion.sec_filing_pipeline_html batch AAPL NVDA TSLA --filing-type 10-K
