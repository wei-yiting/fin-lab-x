...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_pre_run_returns_content_count_excluding_sentinel(
    clean_collection, mock_openai_embed
):
    """Pre-run reports the collection and its content-point count.

    The count must exclude the bookkeeping status sentinel that ingestion
    ...
    # Exactly one sentinel point (status=complete) is excluded from the content count.
