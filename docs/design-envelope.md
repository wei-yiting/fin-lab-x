...
| Concurrent JIT of same ticker | May happen | Every same-ticker ingest attempt runs the branch-free §2 sequence (guard → wipe → ingest → mark); concurrent attempts coalesce or get a legible "ingestion in progress" response. No distributed locks, no job queues. |
...
- **JIT failure semantics: legible failure is the standard.** Arbitrary tickers WILL hit unsupported cases (foreign filers, missing XBRL, no EDGAR presence). Required behavior: a structured, user-facing explanation of what failed and why. Silent partial/empty answers are bugs; exhaustively *handling* every filing variant is over-engineering. Ingestion is all-or-nothing per ticker — the invariant is **committed or absent**: neither a failed nor an in-flight ingest may ever yield a false-complete marker, partial retrievable state, or a mix of generations — including residue from an earlier abandoned attempt. The sanctioned mechanism is one branch-free sequence for every ingest, first-time, retry, and refresh alike: **guard → wipe → ingest → mark**. Enter the per-ticker in-process in-flight guard before touching anything (concurrent same-ticker attempts coalesce or get a legible "ingestion in progress" response); unconditionally delete the marker (if any) and all per-ticker content; ingest; write the completion marker last. Retrieval treats only marker-complete tickers as present, so the ticker legibly reads as not-ingested while work is in flight. The guard is session-busy-guard tier — an in-process set, not a distributed lock, queue, or version/generation machinery; no cross-store transactions. Proving the invariant under the implementation's actual interleavings belongs to the implementing slice's design and tests, not to this document.
