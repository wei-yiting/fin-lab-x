...
| LLM cost exposure | Chat LLM: **BYOK** (user-supplied key, jin-t pattern) or free tier. Server-side spend (embeddings for JIT ingest, Finnhub) exists on every request. | One global **free-tier daily cap** — requests beyond it are refused with a clear message. A cost guard, not rate-limiting machinery — multi-tenant quota systems stay out of scope. |
...
- **External API failures (EDGAR, Finnhub, LLM providers): single retry + legible error.** One deliberate exception: **upstream rate-limit signals are honored once, in the shared client layer, per the source's semantics** — short-window limits (Finnhub free tier 60/min) get one bounded `Retry-After` backoff; block-style limits (EDGAR 429 ≈ 10-minute IP block, where immediate retry extends the block) are honored pre-emptively via client-side throttling under the source's rate cap, and a 429 fails fast with the block surfaced legibly. No other backoff ladders, no circuit breakers, no fallback providers, no jitter tuning.
- **JIT failure semantics: legible failure is the standard.** Arbitrary tickers WILL hit unsupported cases (foreign filers, missing XBRL, no EDGAR presence). Required behavior: a structured, user-facing explanation of what failed and why. Silent partial/empty answers are bugs; exhaustively *handling* every filing variant is over-engineering. Ingestion is all-or-nothing per ticker — **committed or absent**: a failed, concurrent, or abandoned ingest must never leave retrieval seeing partial or stale-mixed data. How that is achieved belongs to the implementing slice's design and tests, within §1's concurrency rules.
...
| **Architecture decision records** | Every non-obvious decision recorded in `docs/adr/` — one file per decision (`NNNN-slug.md`; decision + rejected alternatives + why). Each ADR opens with a 1–2 sentence decision statement; length follows the decision's complexity, up to about **one page (~500 words)**. Overflow triggers a structural test before any compression of rationale: two fused decisions → split into separate ADRs; design detail → link out to a separate doc (the decision must stand alone without it). With reviewer judgment, a wicked problem — one requiring argued trade-offs across ≥2 real options — may run longer (see ADR-0009). Long-lived; entries are never edited after the fact — a reversed decision gets a new ADR that supersedes the old by number. `design.md` files remain per-feature and disposable. Where this envelope reduced robustness, the ADR cites §9. |
