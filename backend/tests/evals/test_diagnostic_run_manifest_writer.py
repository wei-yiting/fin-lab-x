...
def test_write_run_manifest_csv_rejects_missing_required_identity_columns(
    tmp_path: Path,
) -> None:
...
def test_write_run_manifest_csv_rejects_original_output_columns(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="original columns must not include output"):
        write_run_manifest_csv(
            scenario_name="near_v1_diagnostic",
            output_dir=tmp_path,
            original_columns=["id", "output.response"],
            original_rows=[{"id": "1", "output.response": "oops"}],
            manifest_rows=[
                {
                    "row_id": "1",
                    "session_id": "near_v1_diagnostic::smoke::1",
