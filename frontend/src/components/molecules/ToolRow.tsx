...
type ToolRowState = "input-streaming" | "input-available" | "output-available" | "output-error" | "aborted"
...
function stateLabel(state: ToolRowState, toolName: string, progressText?: string, friendlyTitle?: string): string {
  switch (state) {
    case "input-streaming":
    case "input-available":
...
export function ToolRow({ visualState, toolName, progressText, friendlyTitle }: {
  ...
  return (
    <div className="flex items-center gap-2 py-1.5 px-2 text-sm">
      ...
      <span className="truncate text-muted-foreground">{stateLabel(visualState, toolName, progressText, friendlyTitle)}</span>
