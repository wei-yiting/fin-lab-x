...
| Concurrent JIT of same ticker | May happen | Concurrent attempts must resolve *legibly* (coalesce, or a clear "ingestion in progress" response) within the §2 machinery ceiling — the protocol is designed in the implementing slice. No distributed locks, no job queues. |
...
- **JIT failure semantics: legible failure is the standard.** Arbitrary tickers WILL hit unsupported cases (foreign filers, missing XBRL, no EDGAR presence). Required behavior: a structured, user-facing explanation of what failed and why. Silent partial/empty answers are bugs; exhaustively *handling* every filing variant is over-engineering. Ingestion is all-or-nothing per ticker — the invariant is **committed or absent**: retrieval must never see a false-complete marker, partial state, or a mix of generations (including residue from an abandoned attempt). Machinery ceiling for meeting it: an in-process in-flight check and a completion marker are sanctioned (they are state, not locks); distributed locks, queues, generation/version machinery, and cross-store transactions are not. The concrete ingest protocol and its proof under real interleavings belong to the implementing slice's design and tests, not to this document.
