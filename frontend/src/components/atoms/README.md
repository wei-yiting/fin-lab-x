...
- **Completion announcement** — a single `role="status" aria-live="polite"` region, inlined directly in `ChatPanel` (not a separate atom), announces only the natural-completion event ("Response complete"). Errors are announced separately by `ErrorBlock`'s `role="alert"`, so a failure is never read out twice.
