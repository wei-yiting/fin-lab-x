    ...
    http.post('/api/v1/chat', ({ request }) => {
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          controller.enqueue(encoder.encode(sseFrame({ type: 'text-start', id: 't1' })))
          controller.enqueue(encoder.encode(sseFrame({ type: 'text-delta', id: 't1', delta: 'Looking up...' })))

          // Keep streaming slowly to give time to click stop
          for (let i = 0; i < 30; i++) {
            await new Promise(r => setTimeout(r, 100))
            if (request.signal.aborted) return
            controller.enqueue(encoder.encode(sseFrame({ type: 'text-delta', id: 't1', delta: '.' })))
          }
          controller.enqueue(encoder.encode(sseFrame({ type: 'text-end', id: 't1' })))
          controller.enqueue(encoder.encode(sseFrame({ type: 'finish' })))
          controller.close()
    ...
    http.post('/api/v1/chat', ({ request }) => {
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          controller.enqueue(encoder.encode(sseFrame({ type: 'text-start', id: 't1' })))
          controller.enqueue(encoder.encode(sseFrame({ type: 'text-delta', id: 't1', delta: 'streaming content here' })))

          for (let i = 0; i < 30; i++) {
            await new Promise(r => setTimeout(r, 100))
            if (request.signal.aborted) return
            controller.enqueue(encoder.encode(sseFrame({ type: 'text-delta', id: 't1', delta: ` chunk${i}` })))
          }
          controller.enqueue(encoder.encode(sseFrame({ type: 'text-end', id: 't1' })))
          controller.enqueue(encoder.encode(sseFrame({ type: 'finish' })))
          controller.close()
