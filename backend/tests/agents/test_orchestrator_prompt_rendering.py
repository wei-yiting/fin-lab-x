...
from backend.agent_engine.agents.base import _DEFAULT_SYSTEM_PROMPT, Orchestrator
...
EXPECTED_TOOLS_BY_PROFILE = {
    # reader carries the RAG search tool (ADR-0018; PRD Phase 2 — baseline
...
def test_reader_system_prompt_advertises_search_and_citation_contract():
    ...
    citation contract (inline [N], stable sec:// IDs instead of model-written
    URLs, evidence gap annotations, pinpoint/synoptic routing per ADR-0010,
    fiscal-year identity from the tool result).
    """
    text = READER_PROMPT_PATH.read_text()
    assert "sec_filing_search" in text
    assert "CITATION BY SOURCE TYPE" in text
    assert "sec://" in text
    # SEC-sourced claims never carry a model-written URL (the UI builds links
    # from the tool artifact) — the single most important contract line.
    assert "NEVER write a URL for any SEC-sourced claim" in text
    ...
    assert "sec_filing_list_sections" in text
    assert "sec_filing_get_section" in text
    # Fiscal-year identity comes from the tool result (fiscal_year_end), and
    # the model is told FY ≠ calendar year.
    assert "fiscal_year_end" in text
    assert "NOT the calendar year" in text
    # The tool no longer emits a per-call ordinal; the prompt must not refer
    # to one (the model owns [N] numbering across calls).
    assert "`n`" not in text


# ---------------------------------------------------------------------------
# Cross-prompt contract guard. baseline and reader deliberately diverge (baseline
# is the A/B control; reader rewrites must not flow back), so the prompts are
# kept as separate copies rather than templated. The few sections that are
# code-coupled contracts — not style — must stay identical in every copy,
# including the in-code fallback used by placeholder profiles.
# ---------------------------------------------------------------------------

_PROMPT_SOURCES = {
    "baseline": lambda: BASELINE_PROMPT_PATH.read_text(),
    "reader": lambda: READER_PROMPT_PATH.read_text(),
    "_DEFAULT_SYSTEM_PROMPT": lambda: _DEFAULT_SYSTEM_PROMPT,
}

# (contract string, what it is coupled to)
_CODE_COUPLED_CONTRACTS = [
    # RunBudgetMiddleware's exhaustion ToolMessage wording (base.py) — the
    # prompt tells the model how to interpret exactly this string.
    ('"Per-run tool-call budget reached"', "RunBudgetMiddleware ToolMessage"),
    # Substituted by Orchestrator._render_prompt at startup.
    ("{max_tool_calls_per_run}", "_render_prompt template variable"),
    # Reference-definition syntax the frontend parser (markdown-sources.ts)
    # recognises as CommonMark definitions.
    ('[1]: <url> "<title>"', "frontend markdown-sources.ts definition parser"),
    # Half-width bracket rule — full-width markers break the same parser.
    ("NEVER full-width【1】", "frontend markdown-sources.ts definition parser"),
]


@pytest.mark.parametrize("source_name", sorted(_PROMPT_SOURCES))
@pytest.mark.parametrize(("contract", "coupled_to"), _CODE_COUPLED_CONTRACTS)
def test_code_coupled_prompt_contracts_present_in_every_prompt_source(
    source_name, contract, coupled_to
):
    text = _PROMPT_SOURCES[source_name]()
    assert contract in text, (
        f"{source_name} prompt lost the code-coupled contract {contract!r} "
        f"(coupled to: {coupled_to})"
    )
