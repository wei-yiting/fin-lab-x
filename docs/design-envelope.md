...
| **Architecture decision records** | Every non-obvious decision recorded in `docs/adr/` — one file per decision (`NNNN-slug.md`; decision + rejected alternatives + why). Each ADR opens with a 1–2 sentence decision statement and stays as short as its decision allows — a few sentences is a valid ADR; **one page (~500 words)** is a soft ceiling, not a length to fill. Overflow triggers a structural test before any compression of rationale: two fused decisions → split into separate ADRs; design detail → link out to a separate doc (the decision must stand alone without it). With reviewer judgment, a wicked problem — one requiring argued trade-offs across ≥2 real options — may exceed the ceiling (see ADR-0009). Long-lived; entries are never edited after the fact — a reversed decision gets a new ADR that supersedes the old by number. `design.md` files remain per-feature and disposable. Where this envelope reduced robustness, the ADR cites §9. |
