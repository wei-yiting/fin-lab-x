...
- `sec_filing_search.py`: Implements `sec_filing_search` — wraps the dense vector retriever (`sec_dense_pipeline/retriever.py`) for pinpoint/RAG queries over a filing, returning evidence chunks with stable citation IDs (ADR-0019). EDGAR URLs are built directly from the retriever's denormalized `cik`/`accession_number`/`primary_document` chunk fields, no separate filing-store lookup. Reader-profile only.
