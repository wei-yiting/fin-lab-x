...
const toolOutputPatterns: Array<{ pattern: RegExp; title: string; retriable: boolean }> = [
  { pattern: /rate limit/i, title: "Too many requests. Please wait a moment and try again.", retriable: true },
  ...
  { pattern: /permission denied|forbidden/i, title: "Access denied for this resource.", retriable: false },
]

const midStreamPatterns: Array<{ pattern: RegExp; title: string; retriable: boolean }> = [
  { pattern: /context length exceeded|token limit/i, title: "This conversation is too long. Start a new chat to continue.", retriable: false },
  { pattern: /rate limit/i, title: "The system is busy. Please try again in a moment.", retriable: true },
]
...
export function toFriendlyError(ctx: ErrorContext): FriendlyError {
  ...
  switch (ctx.source) {
    ...
    case "network":
      return { title: "Connection lost. Check your network and try again.", retriable: true, detail }
