...
| ---------- | ------- | ------ | ----------------- | ----- |
| `braintrust` | latest | Context7 `/websites/braintrust_dev` | `Eval()` API：接受 `data` (list/lambda), `task` (callable), `scores` (list), `no_send_logs` (bool)。回傳 `.results` 和 `.summary` | Python scorer 可回傳 `float` 或 `{"name": str, "score": float}` |
...
| Create | `backend/evals/scenarios/language_policy/config.yaml` | Column mapping + scorer 清單 |
| Create | `backend/evals/config.py` | 載入 scenario config 的 Pydantic models + YAML parser |
| Create | `backend/evals/loader.py` | CSV 讀取 + column mapping 轉換邏輯 |
| Create | `backend/evals/scorer_registry.py` | 從 dotpath 解析 scorer function + LLM-judge rubric builder |
| Create | `backend/evals/runner.py` | Eval Runner：scenario discovery + 組裝 `Eval()` + result CSV 輸出 |
| Create | `backend/evals/scorers/language.py` | Language policy scorers：`tool_arg_no_cjk`, `response_language` |
| Create | `backend/evals/tasks.py` | Task functions：wrapping `Orchestrator.run()` for Braintrust `Eval()` |
| Create | `backend/evals/braintrust_config.yaml` | Braintrust 專案層級設定（project name, API key env var, local_mode） |
| Create | `backend/tests/evals/test_config.py` | config.py 的 unit tests |
| Create | `backend/tests/evals/test_loader.py` | loader.py 的 unit tests |
| Create | `backend/tests/evals/test_scorer_registry.py` | scorer_registry.py 的 unit tests |
| Create | `backend/tests/evals/test_runner.py` | runner.py 的 integration tests |
...
backend/evals/
  ...
  config.py                       # Pydantic models for scenario config
  loader.py                       # CSV + column mapping
  scorer_registry.py              # Scorer resolution + LLM-judge builder
  runner.py                       # CLI entry point + Eval() assembly
  tasks.py                        # Task functions (wrapping Orchestrator)
  ...
  scenarios/
    language_policy/
      ...
      config.yaml                 # column mapping + scorer refs
  scorers/
    __init__.py
    language.py                   # tool_arg_no_cjk, response_language
...
- `braintrust` 和 `autoevals` 加到 `[project.optional-dependencies] dev` 區塊
...
| ----- | ------- | --------------- | --- |
| Dependency | `cd /Users/dong.wyt/Documents/dev-projects/fin-lab-x-wt-feat-csv-eval-management && pip install -e ".[dev]"` | 安裝成功，`import braintrust` 和 `import autoevals` 不報錯 | 確認 SDK 可用 |
| Lint | `ruff check backend/evals/` | 無 error | 確認 code style |

**Execution Checklist:**

- [ ] 更新 `pyproject.toml` 新增 `braintrust` 和 `autoevals` 到 dev dependencies
- [ ] 更新 `.gitignore` 新增 `backend/evals/results/`
- [ ] 建立 `backend/evals/braintrust_config.yaml`
- [ ] 執行 `pip install -e ".[dev]"` 確認安裝成功
- [ ] 執行 `python -c "import braintrust; import autoevals; print('OK')"` 確認 import 正常
...
- Tests: `backend/tests/evals/__init__.py`, `backend/tests/evals/test_config.py`
...
class TaskConfig(BaseModel):
    function: str                     # Python dotpath, e.g. "backend.evals.tasks.run_v1"
...
| Scope | Command | Expected Result | Why |
| ----- | ------- | --------------- | --- |
| Targeted | `python -m pytest backend/tests/evals/test_config.py -v` | All tests pass | Config parsing 正確 |
| Type check | `pyright backend/evals/config.py` | 0 errors | Type safety |

**Execution Checklist:**

- [ ] 🔴 建立 `backend/tests/evals/__init__.py` 和 `backend/tests/evals/test_config.py`，撰寫測試
- [ ] 🔴 執行測試確認 **fail**（RED）
- [ ] 🟢 實作 `backend/evals/config.py`
...
- Tests: `backend/tests/evals/test_loader.py`
...
| Scope | Command | Expected Result | Why |
| ----- | ------- | --------------- | --- |
| Targeted | `python -m pytest backend/tests/evals/test_loader.py -v` | All tests pass | CSV 轉換正確 |

**Execution Checklist:**

- [ ] 🔴 撰寫 `backend/tests/evals/test_loader.py` 測試（含 fixture CSV 檔案或 `io.StringIO`）
- [ ] 🔴 執行測試確認 **fail**（RED）
- [ ] 🟢 實作 `backend/evals/loader.py`
- [ ] 🔵 Review + refactor，重跑測試確認 **pass**
...
- Create: `backend/evals/scorers/language.py`
...
from backend.evals.config import ScorerConfig
...
```python
# backend/evals/scorers/language.py
...
| Type check | `pyright backend/evals/scorer_registry.py backend/evals/scorers/language.py` | 0 errors | Type safety |

**Execution Checklist:**

- [ ] 🔴 撰寫 `backend/tests/evals/test_scorer_registry.py` 測試
- [ ] 🔴 執行測試確認 **fail**（RED）
- [ ] 🟢 實作 `backend/evals/scorer_registry.py`、`backend/evals/scorers/__init__.py`、`backend/evals/scorers/language.py`
- [ ] 🔵 Review + refactor，重跑測試確認 **pass**
...
- Create: `backend/evals/tasks.py`
- Create: `backend/evals/scenarios/language_policy/dataset.csv`
- Create: `backend/evals/scenarios/language_policy/config.yaml`
...
```yaml
# backend/evals/scenarios/language_policy/config.yaml
name: language_policy
csv: dataset.csv

task:
  function: backend.evals.tasks.run_v1
...
scorers:
  - name: tool_arg_no_cjk
    function: backend.evals.scorers.language.tool_arg_no_cjk
  - name: response_language
    function: backend.evals.scorers.language.response_language
```
...
| Scope | Command | Expected Result | Why |
| ----- | ------- | --------------- | --- |
| Targeted | `python -c "from backend.evals.tasks import run_v1; print(type(run_v1))"` | `<class 'function'>` | Task function 可 import |
| Config parse | `python -c "from backend.evals.config import load_scenario_config; from pathlib import Path; c = load_scenario_config(Path('backend/evals/scenarios/language_policy/config.yaml')); print(c.name)"` | `language_policy` | Scenario config 可載入 |

**Execution Checklist:**

- [ ] 建立 `backend/evals/scenarios/language_policy/` 目錄
- [ ] 建立 `dataset.csv`，從現有 `LANGUAGE_POLICY_CASES` 轉換
- [ ] 建立 `config.yaml`，定義 column mapping 和 scorer refs
- [ ] 實作 `backend/evals/tasks.py`
...
- Tests: `backend/tests/evals/test_runner.py`
...
Usage:
    python -m backend.evals.runner language_policy
    python -m backend.evals.runner --all
    python -m backend.evals.runner language_policy --local-only
"""
import argparse
...
def discover_scenarios(scenarios_dir: Path) -> list[str]:
    """掃描 scenarios/ 下有 config.yaml 的子目錄，回傳 scenario names。"""
...
- `discover_scenarios`：空目錄 / 缺 config.yaml → 不回傳該目錄（edge case）
...
| Scope | Command | Expected Result | Why |
| ----- | ------- | --------------- | --- |
| Targeted | `python -m pytest backend/tests/evals/test_runner.py -v` | All tests pass | Runner 邏輯正確 |
| Discovery | `python -c "from backend.evals.runner import discover_scenarios; from pathlib import Path; print(discover_scenarios(Path('backend/evals/scenarios')))"` | `['language_policy']` | Scenario discovery 正確 |

**Execution Checklist:**

- [ ] 🔴 撰寫 `backend/tests/evals/test_runner.py` 測試（mock task function，`local_only=True`）
- [ ] 🔴 執行測試確認 **fail**（RED）
- [ ] 🟢 實作 `backend/evals/runner.py`
- [ ] 🔵 Review + refactor，重跑測試確認 **pass**
...
| --- | --- | --- | --- |
| 1 | CLI | `python -m backend.evals.runner language_policy --local-only` | 執行完成，terminal 顯示 eval summary（每個 scorer 的 pass/fail 統計） |
...
| 4 | Re-run | 再跑一次 `python -m backend.evals.runner language_policy --local-only` | 產生第二個 result CSV（不覆蓋第一個） |
...
- Update: `backend/evals/runner.py`（移除 `local_only` hardcode，接入 Braintrust config）
...
| Scope | Command | Expected Result | Why |
| ----- | ------- | --------------- | --- |
| Targeted | `python -m backend.evals.runner language_policy` | Terminal 輸出 Braintrust experiment URL | 確認上傳成功 |
...
- [ ] 執行 `python -m backend.evals.runner language_policy`
...
| # | Method | Step | Expected Result |
| --- | --- | --- | --- |
| 1 | CLI | `python -m backend.evals.runner language_policy` | Terminal 輸出 experiment URL (run 1) |
...
| 4 | CLI | 再跑一次 `python -m backend.evals.runner language_policy` | Terminal 輸出新的 experiment URL (run 2) |
